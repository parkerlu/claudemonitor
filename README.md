...
>
两个壳共用同一套引擎：iPhone 上是 iMessage 插件，Mac 上是菜单栏 App。
...
macOS **没有** iMessage 扩展这个东西 —— `Messages.framework` 在 iOS 头文件里写着
`API_UNAVAILABLE(macos)`。所以 Mac 上不做插件，做成菜单栏 App：浮在你正在打字的窗口之上，
写完直接粘进去。副作用是它在 Slack、Mail、浏览器里一样能用，不只限于 Messages。

```
⌥Space 唤出浮窗 → 打中文 → 英文出现 → ⌘↩（或点一下英文）
  → 浮窗消失，焦点回到刚才那个 App，英文已粘好 → 回车发送
```

浮窗里：`⌘1`–`⌘4` 切语气，`Esc` 收起。

- 首次启动会要「辅助功能」权限，那是模拟 ⌘V 用的。不给也能用，英文仍然进剪贴板，自己按 ⌘V。
